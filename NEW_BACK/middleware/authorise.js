...
function authoriseViewPublic(req, res, next) {
    req.accessType = 'view';
...
function authoriseViewPrivate(req, res, next) {
    req.accessType = 'view';
...
async function authoriseEdit(req, res, next) {
    try {
        const token = getToken(req);
        if (!token) throw 401;
...
module.exports = { authoriseViewPublic, authoriseViewPrivate, authoriseEdit, authoriseAdmin }